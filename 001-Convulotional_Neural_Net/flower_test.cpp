...
// --- CONFIGURATION ---
const std::string loadModelName = "FlowerAmpota_RGB0";
const std::string csvPath = "cnn-data/flowers/flowers_dataset.csv";
...
void evaluateFlowers(Sequential& model, const ImageLoader::Dataset& data, int visCount, bool showIncorrectOnly) {
    int correct = 0;
    int displayed = 0;
    ...
    for(size_t i = 0; i < data.inputs.size(); i++) {
        ...
        if (displayed < visCount) {
            ...
            std::cout << "Pred: " << getFlowerName(predictedClass) << " (" << (int)confidence << "%)";
            std::cout << " | Actual: " << getFlowerName(actualClass) << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if(argc < 2) {
        ...
        std::cout << "  1. Test single image:  ./flower_test <path_to_image>" << std::endl;
        std::cout << "  2. Test dataset:       ./flower_test <count> <show_incorrect_only>" << std::endl;
        return -1;
    }
    ...
    try {
        ...
        // --- MODE 2: Dataset Evaluation ---
            ...
            evaluateFlowers(model, testData, visCount, showIncorrect);
        }
