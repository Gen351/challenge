...
int main(int argc, char* argv[]) {
    ...
    if(argc < 2) {
        ...
        std::cout << "  1. Test single image:  ./catdog_test <path_to_image> <show_filters(true | false)>" << std::endl;
        std::cout << "  2. Test dataset:       ./catdog_test <count(-1 for all)>" << std::endl;
        return -1;
    }
    ...
    if (!loadModelName.empty()) {
        ...
        try {
            DataLoader::ModelIO::loadModel(model, "catdog/" + loadModelName, "../../trained_cnns/");
